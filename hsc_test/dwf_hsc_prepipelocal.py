...
def dwf_prepipe_qsubccds(files,path_to_watch,path_to_unzip,path_to_mary,path_to_qsub,HSC_DIR, rerun):
	...
	qsub_file.write('if [ -f /lfs/data0/fstars/4.0.5/bashrc ]; then\n')
	qsub_file.write('echo "Using Local-Node Pipeline"\n')
	qsub_file.write('source /lfs/data0/fstars/4.0.5/bashrc\n')
	qsub_file.write('setup -j astrometry_net_data ps1_pv3_3pi_20170110-and\n')
	qsub_file.write('else')
	qsub_file.write('echo "Using Lustre Pipeline"\n')
	...
	for f in files:
		...
		mjd_id=str(int(math.floor(mjd-55927)))
		while(len(mjd_id) < 5):
			mjd_id='0'+mjd_id 
		while(len(vist) < 7):
			visit='0'+visit
		...
		#print([visit,ccd])
		com_script='('
		com_script=com_script+'sleep '+str(0.5*n)+ ' ; '

		qsub_file.write('mkdir $PBS_JOBFS/dwf/raw/\n')
		path_to_unzip='$PBS_JOBFS/dwf/raw/'

		rerun_dir='$PBS_JOBFS/dwf/'+rerun
		qsub_file.write('mkdir {0} \n'.format(rerun_dir))

		image=path_to_unzip+f[:-3]
		com_script=com_script+'time funpack -O {1} {0}; '.format(im,image)

		com_script=com_script+'time hscIngestImages.py {0} {1} --mode=link --no-backup-config ;'.format(HSC_DIR,image)
		com_script=com_script+'time hscProcessCcd.py {0} --rerun {1} -C {4} -c isr.fwhm={5} -c isr.doBrighterFatter=False --id visit={2} ccd={3} --no-backup-config --clobber-config'.format(HSC_DIR,rerun_dir,visit,ccd,config_file,psf_val)#  --clobber-config
		#com_script=com_script+'time hscProcessCcd.py {0} --rerun {1} --id visit={2} ccd={3}  --clobber-config'.format(HSC_DIR,rerun,visit,ccd)
		#com_script=com_script+'time hscProcessCcd.py {0} --rerun {1} -c isr.fwhm={4} --id visit={2} ccd={3} --no-backup-config --clobber-config'.format(HSC_DIR,rerun_dir,visit,ccd,psf_val)
		
		corr_dir=HSC_DIR+'rerun/'+rerun+'/'+mjd_id+'/'+filt+'/corr/'
		corr_name='CORR-'+visit+'-'+ccd_num+'.fits'
		
		com_script=com_script+' ; cp {0} {1}'.format(rerun_dir+'/'+mjd_id+'/'+filt+'/corr/'+corr_name,corr_dit+corr_name)
